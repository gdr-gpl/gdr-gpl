const JSON_INDEX_URL=`${window.BASE_URL}index.json`,QUERY_URL_PARAM="query",MAX_HITS_SHOWN=10,FUSE_OPTIONS={keys:[{name:"title",weight:.8},{name:"description",weight:.5},{name:"content",weight:.3}],ignoreLocation:!0,includeMatches:!0,includeScore:!0,minMatchCharLength:2,threshold:.2};let fuse;const getInputEl=()=>document.querySelector("#s"),initFuse=e=>{fuse=new Fuse(e,FUSE_OPTIONS)},getQuery=()=>getInputEl().value.trim(),setUrlParam=e=>{const t=new URL(location.origin+location.pathname);t.search=`${QUERY_URL_PARAM}=${encodeURIComponent(e)}`,window.history.replaceState({},"",t)},doSearchIfUrlParamExists=()=>{const e=new URLSearchParams(window.location.search);if(e.has(QUERY_URL_PARAM)){const t=decodeURIComponent(e.get(QUERY_URL_PARAM));getInputEl().value=t,handleSearchEvent()}},createHitHtml=e=>{const t=e.item,n=t.url,a=t.title,s=t.description;console.log(s);const o=t.date?new Date(t.date).toLocaleDateString():"",i=t.categories||[];return`
    <article style="border-bottom:1px solid #eee;">
     <h1><a href="${window.BASE_URL}${n}" style="text-decoration:none; color:#222;">${a}</a></h1>
       ${o?`<p style="color: #666;font-size: 10px;letter-spacing: 0.1em;">Posted on ${o}</p>`:""}
      <p>${s} ${t.readmore?`... <a href="${window.BASE_URL}${n}">Continue reading →</a>`:""}</p>
